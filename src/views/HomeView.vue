...
function formatTime(seconds: number): string {
  ...
  if (h > 0) return `${h}${hrStr} ${m}${minStr}`
  return `${m}${minStr}`
}
</script>
...
<template>
  <div class="container fade-in">
    ...
    <div v-if="loading" class="center-screen">
      ...
      <p class="neon-text">{{ t('loading') }}</p>
    </div>

    <!-- Error -->
    <div v-else-if="error" class="center-screen">
      <p style="color: var(--accent-red)">{{ error }}</p>
      <button class="btn-neon" @click="router.push('/onboarding')">{{ t('initialize') }}</button>
    </div>
    ...
    <div v-else-if="user" class="content">
      ...
      <div class="header">
        <div class="greeting">
          ...
          <h2>{{ user.is_admin ? t('sysadmin') : t('hello_user', { name: user.nickname.toUpperCase() }) }}</h2>
        </div>
        <div class="badge-online">
          {{ onlineCount }} {{ t('online').toUpperCase() }}
        </div>
      </div>
        ...
        <div class="user-info">
          ...
          <div class="stats-row">
            ...
            <span class="stat reputation" :style="{ color: user.reputation_percent >= 70 ? 'var(--accent-green)' : 'var(--accent-red)' }">
              {{ t('rep') }} {{ user.reputation_percent }}%
            </span>
          </div>
          <div class="time-stat">{{ t('uptime') }} {{ formatTime(user.total_chat_seconds) }}</div>
        </div>
